...
# Modelin derlenmesi
model.compile(optimizer=Adam(), loss='sparse_categorical_crossentropy', metrics=['accuracy', 'rms'])

# Modeli eğit
model.fit(X_train, y_train_encoded, epochs=50, batch_size=32, validation_split=0.1)

# Modeli değerlendir
loss, rms = model.evaluate(X_test, y_test_encoded)
print(f'Test setindeki doğruluk: {rms}')
